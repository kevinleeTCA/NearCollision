/*
	...
	This is an implemention of offline stage in NCA-2.0
*/

#include "stdafx.h"
#include "head.h"

//穷举所有Hamming重量小于d的53比特的内部状态差分，然后在N大小的样本空间下，进行预计算，输出密钥流长度为l。
/*
	d:输入状态差分的最大汉明重量
*/
void offLine_table_construct(u32 d){
	...
	for(int i=1;i<=d;i++){
		...
		double time[4]={0};
		start_cal();
		combination_for_search_grain_reduce(state_Len,i,curr_DIR);
		end_cal(time);
...
void combination_for_search_grain_reduce(u32 n,u32 k,string curr_DIR){
	...
	genOutput_diff_imp_grain_reduce(k,v,curr_DIR);
	//穷举C(n,k)的所有组合，为每个组合设定输入状态差分，并执行grain得到输出差分
	while(combin_for_search_imp_sub_grain_reduce(k,v,curr_DIR)){
		if((++counter) % 20000 ==0){
...
void genOutput_diff_imp_grain_reduce(u32 k,u32 *v,string curr_DIR){
	...
	*/
	...
	string inputDiffStr=char2HexString(inputDiffbyte,STATE_BYTE);
	//将data 排序
	//sort(data,data+SIZE_DIFF);
	qsort(data,STATE_NUM,sizeof(unsigned long long),comp);
...
void stateBit2Byte(u8 *stateByte,u32 byteLen,u32 *LFSR,u32 *NFSR,u32 bitLen){
	for(int j=0;j<bitLen;j++){
		u32 idx=j/byteLen;
		u32 rotate=j%byteLen;
		stateByte[idx]+=(NFSR[j]&0x01)<<rotate;
	}
	for(int j=0;j<bitLen;j++){
		u32 idx=j/byteLen+4;
		u32 rotate=j%byteLen;
		stateByte[idx]+=(LFSR[j]&0x01)<<rotate;
	}
}